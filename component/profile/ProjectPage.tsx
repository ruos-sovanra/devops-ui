...
export default function ProjectPage() {
    ...
    const handleServiceDataChange = (service: keyof ServiceData, field: string, value: string) => {
        setServiceData(prev => ({
            ...prev,
            [service]: {
                ...prev[service],
    ...
    const openEnvModal = (projectId: number) => {
        ...
        if (project && project.env) {
            setEnvVars(Object.entries(project.env).map(([key, value]) => ({ key, value })))
        } else {
    ...
    const handleSaveEnv = () => {
        ...
        const envObject = Object.fromEntries(envVars.filter(({key}) => key !== '').map(({key, value}) => [key, value]))
        setProjects(projects.map(project =>
            project.id === selectedProjectId
                ? { ...project, env: envObject }
                : project
        ))
